...
def view_attendance_gui():
    ...
    tk.Label(viewer, text="📜 The Sacred Attendance Scroll 📜", 
             font=("Comic Sans MS", 18, "bold"), bg='#f0e6f6', fg='#4b0082').pack(pady=10)
    ...
    tk.Button(viewer, text="Close Scroll", command=viewer.destroy,
              bg='#ff7f50', fg='#4b0082', font=("Comic Sans MS", 12, "bold"),
              relief='raised', bd=4).pack(pady=15)
...
def stylish_askstring(title, prompt, show=''):
    ...
    entry = tk.Entry(dialog, show=show, font=font, bg='#fff0f5', fg='#4b0082',
                     insertbackground='#4b0082', relief='groove', bd=3)
    ...
    tk.Button(btn_frame, text="Submit",
              ...
              bg='#ffb6c1', fg='#4b0082', font=font, relief='raised', bd=4,
              width=10).pack(side=tk.LEFT, padx=15)

    tk.Button(btn_frame, text="Cancel",
              command=lambda: [result.__setitem__(0, None), dialog.destroy()],
              bg='#87cefa', fg='#4b0082', font=font, relief='raised', bd=4,
              width=10).pack(side=tk.RIGHT, padx=15)
...
def stylish_messagebox(title, message, type='info'):
    ...
    tk.Label(dialog, text=message, bg='#e0f7fa', fg='#00796b',
             font=font, wraplength=380).pack(pady=20)
    btn_frame = tk.Frame(dialog, bg='#e0f7fa')
    btn_frame.pack(pady=10)

    if type == 'yesno':
        result = [None]
        tk.Button(btn_frame, text="Yes",
                  command=lambda: [result.__setitem__(0, True), dialog.destroy()],
                  bg='#ffcc80', fg='#4b0082', font=font, relief='raised',
                  bd=4, width=10).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="No",
                  command=lambda: [result.__setitem__(0, False), dialog.destroy()],
                  bg='#90ee90', fg='#4b0082', font=font, relief='raised',
                  bd=4, width=10).pack(side=tk.RIGHT, padx=10)
        dialog.wait_window()
        return result[0]
    else:
        tk.Button(btn_frame, text="OK", command=dialog.destroy,
                  bg='#ffb6c1', fg='#4b0082', font=font, relief='raised',
                  bd=4, width=10).pack()
        dialog.wait_window()
...
def ensure_attendance_file():
    if not os.path.exists(ATTENDANCE_FILE):
        with open(ATTENDANCE_FILE, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Name","ID","Date","Time","Day"])
...
def mark_attendance(name, numeric_id, is_admin=False, speak=True):
    ensure_attendance_file()
...
def build_dataset_and_label_map():
    ...
    admin_images = [f for f in os.listdir(ADMIN_FOLDER)
...
def block_user():
    ...
    user_to_block = stylish_askstring("Block User",
                                      f"Enter name to block:\nAvailable: {', '.join(names)}")
...
def unblock_user():
    ...
    user_to_unblock = stylish_askstring("Unblock User",
                                        f"Enter name to unblock:\nBlocked: {', '.join(blocked)}")
...
def camera_for_one_user(is_admin=False):
    ...
    # Create a Tkinter window for the camera feed
    camera_window = tk.Toplevel()
    camera_window.attributes('-topmost', True)  # Stay on top
    ...
    # CRITICAL: Bring window to absolute top and give focus
    camera_window.lift()
    ...
    try:
        while not stop_camera[0]:
            ...
            for (x, y, w, h) in faces:
                ...
                if recognizer is not None and len(label_map_int) > 0:
                    try:
                        ...
                        if pred_label in label_map_int and conf < UNKNOWN_THRESHOLD:
                            name_text = label_map_int[pred_label]
                            color = (0, 255, 127)
                            is_known = True
                            is_admin_detected = (name_text == ADMIN_USERNAME)
                            numeric_id = id_map.get(name_text, "UnknownID")

                            if name_text in blocked_users:
                                if name_text not in announced:
                                    speak_async(f"{name_text} has been cursed and cannot enter!")
                                    announced.add(name_text)
                            else:
                                just_marked_now = mark_attendance(
                                    ADMIN_USERNAME if is_admin_detected else name_text,
                                ...
                                if name_text not in announced:
                                    if just_marked_now:
                                        if is_admin_detected:
                                            speak_async(f"{name_text}, mighty admin! Your attendance scroll has been stamped!")
                                            speak_async("Press Q to escape the matrix")
                                        else:
                                            speak_async(f"{name_text}, zap! Your presence has been laser-etched into the system!")
                                    else:
                                        if is_admin_detected:
                                            speak_async(f"{name_text}, mighty admin, your attendance was already marked today.")
                                        else:
                                            speak_async(f"{name_text}, your attendance was already marked today.")
                                    announced.add(name_text)
                    except:
                        pass
                ...
                if not is_known and not is_admin:
                    if buffer_count < SAMPLES_NEW_USER:
                        ...
                        buffer_count += 1
                        cv2.putText(display, "Collecting magical face data...",
                                    (x, y-30),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (173,216,230), 2)
                    if buffer_count >= SAMPLES_NEW_USER:
                        name, numeric_id = register_user(unknown_faces_buffer)
                ...
                cv2.putText(display, name_text, (x, y-10),
            ...
            # CRITICAL: Continuously enforce topmost position
            camera_window.lift()
            camera_window.focus_force()
            
            # Process Tkinter events to keep window responsive
            ...
            try:
                ...
                if not listening_mode:
                    if "hello" not in text_lower:
                        continue
                    listening_mode = True
                    speak_async("erry here! I'm listening. Say 'Over' when you're done.")
                ...
                if ("exit admin" in text_lower or 
                    "close admin" in text_lower or 
                    "quit admin" in text_lower):
                    VOICE_COMMAND_QUEUE.put("exit_admin_panel")

                elif ("open admin panel" in text_lower or 
                      "admin panel" in text_lower):
                    VOICE_COMMAND_QUEUE.put("open_admin_panel")

                elif ("start" in text_lower or
                    "start attendance" in text_lower or
                    "start attendnace" in text_lower):
                    VOICE_COMMAND_QUEUE.put("start_attendance")

                elif ("exit the system" in text_lower or
                      "exit system" in text_lower or
                      "close the system" in text_lower or
                      "quit the system" in text_lower):
                ...
                elif "unblock user" in text_lower or "unblock" in text_lower or "un block" in text_lower or "un" in text_lower:
                    VOICE_COMMAND_QUEUE.put("unblock_user")

                elif ("mark" in text_lower or
                      "mark admin attendance" in text_lower or
                      "mark my attendance" in text_lower):
...
if __name__ == "__main__":
    ...
    black_dialog.update()
    speak_sync("My name is Snow... summoned into existence by Sherry. Let's wander through my little world together.")
